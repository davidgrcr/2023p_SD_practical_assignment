	...
	public List<Operation> listNewer(TimestampVector sum){
		...
		for (String hostId : log.keySet()) {
	        ...
	        for (Operation op : ops) {
	            if (op.getTimestamp().compare(localLastTimestamp) > 0) {
	                newerOperations.add(op);
	            }
	        }
	    }

	    // Sort the newerOperations list by timestamp
	    newerOperations.sort((op1, op2) -> (int) op1.getTimestamp().compare(op2.getTimestamp()));
